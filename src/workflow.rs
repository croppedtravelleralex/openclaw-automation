...
pub fn default_suggestions_for_stage(stage: AutopilotStage) -> Vec<WorkflowSuggestion> {
    match stage {
        ...
        AutopilotStage::CommitPush => vec![
            suggestion("commit 当前稳定成果", 1, "把本轮稳定成果落盘", WorkflowSuggestionKind::Refactor),
            suggestion("评估是否 push", 2, "外发前进入确认点", WorkflowSuggestionKind::Refactor),
        ],
...
pub fn dispatch_top_suggestions(root: &Path, state: &mut ManagedProjectState, max_actions: usize) -> Result<Vec<WorkflowActionRecord>> {
    let executed = state
        .next_suggestions
        .iter()
        .take(max_actions)
        ...
        .collect::<Vec<_>>();
    append_execution_log(root, &executed)?;
...
pub fn run_minimal_cycle_step(root: &Path, config: &ManagedProjectConfig, state: &mut ManagedProjectState) -> Result<()> {
    match state.stage {
        ...
        AutopilotStage::Execute => {
            let executed = dispatch_top_suggestions(root, state, 2)?;
        ...
        AutopilotStage::CommitPush => {
            ...
            state.last_summary = "已完成 commit_push，进入 cooldown".to_string();
            state.stage = AutopilotStage::Cooldown;
            state.next_suggestions = default_suggestions_for_stage(state.stage);
        }
    ...
    if state.loop_iteration >= state.next_report_at {
        state.pending_confirmation.push(format!("已达到第 {} 轮汇报点，建议向用户汇报当前进展", state.loop_iteration));
        state.next_report_at += config.report_every_rounds;
    }
    Ok(())
}
